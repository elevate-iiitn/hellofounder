...
export const PricingWrapper = ({ children, contactHref, className, type = 'waves' }) => (
  <article
    className={cn(
    ...
    {type === 'waves' && (
      <>
        <div
          className={'w-fit h-fit absolute -top-[106px] sm:left-4 -left-0 waves z-0'}>
          <Wave />
        </div>
        <div
          className={'w-fit h-fit absolute -top-[106px] sm:right-4 -right-0 waves z-0'}>
          <Wave />
        </div>
      </>
    )}
...
export const Paragraph = ({
  ...
  <p
    className={cn('sm:text-2xl text-[clamp(0.1rem,20vw,1.25rem)] font-bold', className)}>
    {children}
  </p>
)
